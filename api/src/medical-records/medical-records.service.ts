...
export class MedicalRecordsService {
  ...
  async create(dto: CreateMedicalRecordDto, userId: string) {
    ...
    // Store vital signs linked to the patient and appointment if provided
    if (vitalSigns) {
  ...
  async update(id: string, dto: UpdateMedicalRecordDto, userId: string) {
    ...
    // Update or create vital signs for this appointment if provided
    if (vitalSigns && existing.appointmentId) {
      const existingVitals = await this.prisma.vitalSigns.findFirst({
        where: { appointmentId: existing.appointmentId },
      });

      const vitalsData: Record<string, unknown> = { recordedById: userId };
      ...
      if (existingVitals) {
        await this.prisma.vitalSigns.update({ where: { id: existingVitals.id }, data: vitalsData });
      } else {
        await this.prisma.vitalSigns.create({
          data: {
            patientId: record.patientId,
            appointmentId: existing.appointmentId,
            recordedAt: vitalSigns.recordedAt ? new Date(vitalSigns.recordedAt) : record.visitDate,
            temperature: vitalSigns.temperature,
            heartRate: vitalSigns.heartRate,
            respiratoryRate: vitalSigns.respiratoryRate,
            bloodPressureSystolic: vitalSigns.bloodPressureSystolic,
